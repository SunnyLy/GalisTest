...
public class DropOfWater extends View {
    ...
    private int mCurrentHeight;//当前长轴高度
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.save();
        mOvalRectF.set(0, 0, mShort_DP * 2, mCurrentHeight);//clipRect
        canvas.clipRect(mOvalRectF);
        mPaint.setColor(mBackgroundColor);
        mOvalRectF.set(0+inset, 0+inset, mShort_DP * 2-inset, mCurrentHeight * 2-inset);//drawRect
        canvas.drawOval(mOvalRectF, mPaint);
        canvas.restore();

        if(mTagBitmap!=null){
            canvas.drawBitmap(mTagBitmap,mShort_DP,mCurrentHeight,mPaint);
        }

        canvas.save();
        canvas.translate(0, mCurrentHeight);
     ...
     */
    public void dragOut() {
        final int currentHeight = mCurrentHeight;
        ...
        mDragOutAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                mCurrentHeight = (int) (((currentHeight - mShort_DP) * (Float) animation.getAnimatedValue()) + mShort_DP);
    ...
    public void drag(float f) {
        ...
        mCurrentHeight = (int) (mShort_DP + (mLong_DP - mShort_DP) * f);
        getLayoutParams().height = getWaterHeight();
        requestLayout();
    }
